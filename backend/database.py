...
class DatabaseManager:
    ...
    def get_users_by_fabrication_status(self, status: int) -> List[Dict[str, Any]]:
        ...
        try:
            users = list(self.db.users.find(
                {"fabrication_status": status, "role": "customer"}
            ).sort("created_at", -1))
    ...
    def update_user_fabrication_status(self, user_id: str, status: int) -> bool:
        """Update user's fabrication status"""
        try:
            result = self.db.users.update_one(
                {"_id": ObjectId(user_id)},
                {
    ...
    def get_all_products(self, skip: int = 0, limit: int = 100, category: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def get_products_count(self, category: Optional[str] = None) -> int:
    ...
    # Add to DatabaseManager class in database.py
    ...
    # User analytics by fabrication status
    def get_users_by_fabrication_status(self, status: int) -> List[Dict[str, Any]]:
        """Get users by fabrication status"""
        try:
            users = list(self.db.users.find({
